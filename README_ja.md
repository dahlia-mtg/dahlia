...
macOS ネイティブの文字起こしアプリです。マイクとシステム音声を同時にキャプチャし、リアルタイムまたは録音後にデバイス上で音声認識を行い、オプションで LLM による要約を生成します。

## 機能

- **デュアル音声キャプチャ** — マイク (AVAudioEngine) とシステム音声 (ScreenCaptureKit) を同時に録音
- **オンデバイス文字起こし** — Apple Speech フレームワークによる単一言語のリアルタイム音声認識と、最大2言語が混在するミーティングの録音後バッチ文字起こし
